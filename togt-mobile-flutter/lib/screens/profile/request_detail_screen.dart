...
class _RequestDetailScreenState extends State<RequestDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final items = (listResponse is Map ? listResponse['data'] : listResponse) as List? ?? [];
      Map? match;
      for (final item in items.whereType<Map>()) {
        if (item['id']?.toString() == widget.id) { match = item; break; }
      }
      if (match == null) throw Exception('Request not found');
      if (mounted) setState(() { request = match; history = results[1] is List ? results[1] as List : []; });
