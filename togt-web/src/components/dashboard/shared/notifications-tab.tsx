...
import { AlertTriangle, Bell, CheckCheck, CheckCircle, FileText, PackageOpen, MessageSquareText, Trash2, XCircle, type LucideIcon } from "lucide-react";
...
export function NotificationsTab() {
  ...
  return <div>
    ...
    {visible.length === 0 ? <EmptyState title="No notifications" description="You're all caught up." /> : <div className="space-y-2">{visible.map((notification) => { const Icon = icons[notification.type] ?? Bell; return <div key={notification.id} className={cn("rounded-xl border p-4 transition-colors duration-200", notification.isRead ? "border-gray-100 bg-white" : "border-togt-blue/20 bg-togt-blue/5")}>
      <div className="flex items-start gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-700"><Icon className="h-4 w-4" /></span><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><p className="text-sm font-semibold text-togt-navy">{notification.title}</p><span className="text-[11px] text-gray-400">{relativeTime(notification.sentAt)}</span></div><p className="mt-1 text-sm text-gray-600">{notification.message}</p><div className="mt-2 flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold uppercase text-gray-500">{notification.type.replace(/_/g, " ")}</span>{!notification.isRead && <span className="rounded-full bg-togt-orange/15 px-2 py-0.5 text-[10px] font-bold text-togt-orange">NEW</span>}</div>
